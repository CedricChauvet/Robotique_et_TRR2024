...
void loop() {
  ...
  for(int p = 380; p< 457; p++){
    float A = pulseToAngle(p);
    float B = acos( h/l - cos(A));
    // Serial.println(A);
    Serial.println(B);
    B = 180.0 / pi *B; 
   
    int pulseB = angleToPulse(B);
    pwm.setPWM(3, 0, p);
    //pwm.setPWM(1, 0, pulseB);
    delay(200);
  }

  for(int p = 457; p> 380; p--){
    float A = pulseToAngle(p);
    float B = acos( h/l - cos(A * pi /180.0));
    Serial.println(A);
    //Serial.println(B);

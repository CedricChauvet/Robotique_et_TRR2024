...
void loop() {
  for(float alphaG = 15.0; alphaG < 40.0; alphaG += 0.10){
   
   // femur gauche
   pwm.setPWM(3, 0, angleToPulse(p3+alphaG));
  
   // tibia gauche
  
    float BetaG = acos( h/l - cos(alphaG * pi / 180.0));
    // conversion en degres
    BetaG = BetaG /pi *180;
    pwm.setPWM(1, 0, angleToPulse(p1 - BetaG));
   
   // l'autre jambe
   //femur droit
   float alphaD = 40.0 - (alphaG - 15.0) * (40.0 - 15.0) / (40.0 - 15.0);
   pwm.setPWM(4, 0, angleToPulse(p4 - alphaD));
   
   // tibia droit
   float BetaD = acos( h/l - cos(alphaD * pi / 180.0));
   BetaD = BetaD /pi *180;
   pwm.setPWM(2, 0, angleToPulse(p2 + BetaD));
